...
export default function SecurityPersonnels() {
  ...
  const renderGuardItem = ({ item }: { item: Guard }) => (
    <View className={`p-5 mb-4 rounded-[30px] border shadow-sm ${isDarkMode ? "bg-gm-navy border-gm-gold" : "bg-white border-gray-100"}`}>
      <View className="flex-row items-center">
        ...
        <View className="flex-1 ml-4 flex gap-1">
          <Text className={`text-lg font-bold ${isDarkMode ? "text-gm-gold" : "text-gray-900"}`} numberOfLines={1}>
            {item.name}
          </Text>
          <View className="flex-row items-center mt-1">
            <Mail size={12} color={isDarkMode ? "#A0AEC0" : "#64748b"} />
            <Text className={`text-md ml-1 ${isDarkMode ? "text-slate-300" : "text-gray-500"}`} numberOfLines={1}>{item.email}</Text>
          </View>
          <View className="flex-row items-center mt-0.5">
            <Phone size={12} color={isDarkMode ? "#A0AEC0" : "#64748b"} />
            <Text className={`text-md ml-1 font-medium ${isDarkMode ? "text-slate-300" : "text-gray-500"}`}>
              {item.phone || "No Phone"}
            </Text>
          </View>
        </View>
      ...
      <View className={`mt-4 pt-4 border-t flex-row items-center ${isDarkMode ? "border-slate-800" : "border-gray-50"} ${item.is_on_duty ? " justify-between" : "justify-end"}`}>
        {item.is_on_duty && (
          <View className="flex-1 pr-2">
            <Text className={`text-[10px] uppercase tracking-widest font-bold ${isDarkMode ? "text-gm-gold" : "text-gray-400"}`}>
              Last Known Location
            </Text>
            <View className="flex-row items-center mt-1">
              <MapPin size={12} color={isDarkMode ? "#D4AF37" : "#6366f1"} />
              <Text className={`text-xs ml-1 font-semibold flex-1 ${isDarkMode ? "text-white" : "text-gray-700"}`} numberOfLines={1}>
    ...
    return (
      <View className={`flex-1 justify-center items-center p-6 ${isDarkMode ? "bg-slate-950" : "bg-gray-50"}`}>
        <TouchableOpacity
          ...
          className="bg-slate-900 px-8 py-4 rounded-3xl shadow-sm border border-gm-gold"
        >
    ...
    <View className={`flex-1 p-4 pt-6 pb-20 ${isDarkMode ? "bg-slate-950" : "bg-gray-50"}`}>
      ...
      <View className="flex-row justify-between items-center mb-4">
        <View className="flex-1 mr-2">
          <Text className={`text-2xl font-bold ${isDarkMode ? "text-gray-300" : "text-gray-900"}`}>Security Team</Text>
      ...
      {user?.estate_ids && user.estate_ids.length > 1 && (
        <TouchableOpacity
          ...
          className={`mb-4 flex-row items-center justify-between p-3.5 rounded-2xl border ${
            isDarkMode ? "bg-gm-navy border-gm-gold" : "bg-white border-gray-100"
          } shadow-sm`}
        >
          <View className="flex-row items-center flex-1">
            <MapPin size={14} color={isDarkMode ? "#D4AF37" : "#6366f1"} />
            <Text className={`ml-2 text-xs font-black uppercase tracking-wider ${isDarkMode ? "text-white" : "text-slate-600"} flex-1`} numberOfLines={1}>
      ...
      {/* Search Bar */}
      <View className={`flex-row items-center px-4 py-3 rounded-2xl border shadow-sm mb-4 ${isDarkMode ? "bg-gm-navy border-gm-gold" : "bg-white border-gray-100"}`}>
          ...
          return (
            <TouchableOpacity
              ...
              className={`px-5 py-2.5 rounded-full border ${
                isSelected 
                  ? isDarkMode ? "bg-gm-navy border-gm-gold" : "bg-indigo-600 border-indigo-600" 
                  : isDarkMode ? "bg-gm-charcoal border-slate-800" : "bg-white border-gray-200"
              }`}
            >
              <Text className={`font-bold text-xs ${isSelected ? (isDarkMode ? "text-gm-gold" : "text-white") : "text-gray-500"}`}>
                {item.label}
              </Text>
            </TouchableOpacity>
          );
      ...
      <Modal visible={estatePickerVisible} animationType="slide" transparent={true}>
        <View className="flex-1 justify-end bg-black/50">
          <View className={`${isDarkMode ? "bg-slate-900 border-t border-gm-gold" : "bg-white"} rounded-t-[2.5rem] p-6 max-h-[60%]`}>
            ...
            <Text className={`text-xl font-bold mb-4 ${isDarkMode ? "text-gm-gold" : "text-slate-900"}`}>Select Active Property Context</Text>
            <FlatList
              data={user?.estates || []}
              keyExtractor={(item) => item.id}
              renderItem={({ item }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <View className="ml-3 flex-1">
                      <Text className={`font-bold text-sm ${isSelected && isDarkMode ? "text-gm-gold" : isDarkMode ? "text-white" : "text-slate-800"}`}>{item.name}</Text>
                    </View>
                  </TouchableOpacity>
                );
              }}
            />
            {selectedEstateId && (
              <TouchableOpacity onPress={() => setEstatePickerVisible(false)} className={`mt-2 p-4 rounded-2xl items-center ${isDarkMode ? "bg-gm-charcoal border border-slate-800" : "bg-slate-200"}`}>
      ...
      <Modal visible={!!selectedImage} transparent animationType="fade" onRequestClose={() => setSelectedImage(null)}>
        <Pressable className="flex-1 bg-black/90 justify-center items-center" onPress={() => setSelectedImage(null)}>
          ...
          {selectedImage && (
            <Image source={{ uri: selectedImage }} style={{ width: width * 0.9, height: width * 0.9 }} className={`rounded-3xl ${isDarkMode ? "bg-gm-navy border border-gm-gold" : "bg-gray-800"}`} resizeMode="cover" />
          )}
        </Pressable>
      </Modal>
    </View>
