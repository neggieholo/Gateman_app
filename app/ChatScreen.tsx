...
const ChatManager = () => {
  ...
  const launchCamera = async (mediaType: any) => {
    try {
      ...
      const webUrl = await getS3UploadedUrl(localUri, "Chat-uploads");
  ...
  const pickDocument = async () => {
    ...
    try {
      const webUrl = await getS3UploadedUrl(asset.uri, "Chat-uploads");
      if (webUrl) {
        setMessages((prev) => prev.filter((m) => m._id !== tempId));
        onSend([
          {
  ...
  const stopAudioRecording = async () => {
    ...
    try {
      ...
      if (uri) {
        ...
        const remoteUrl = await getS3UploadedUrl(uri, "Chat-uploads");
        ...
        >
          ...
          >
            <Image
              source={{
                uri:
                  (typeof selectedTenant?.avatar === "object" &&
                  selectedTenant.avatar
                    ? selectedTenant.avatar[selectedEstateId] ||
                      Object.values(selectedTenant.avatar)[0]
                    : selectedTenant?.avatar) ||
                  "https://via.placeholder.com/300",
              }}
