...
const InviteGuestForm = ({
    ...
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          ...
          "GateMan needs contact permissions to pull phone numbers quickly. Please enable it in your device settings.",
          [
      ...
      // LAUNCHES NATIVE MODAL SELECTION
      ...
      if (contact) {
        ...
        const phoneArray =
          contact.phoneNumbers || (contact as any).phones || [];

        if (phoneArray.length > 0) {
          // 2. Scan through to find the first item containing a real string value
          const validPhoneEntry = phoneArray.find(
            (entry: any) =>
              entry &&
          ...
          const rawNumber = validPhoneEntry ? validPhoneEntry.number : null;

          if (!rawNumber) {
            // console.log(
            //   "Execution stopped: No valid number string found anywhere in the array.",
            // );
            Alert.alert(
              "Notice",
          ...
          // console.log(
          //   "SUCCESS! Found valid number text through array scan:",
          //   rawNumber,
          // );
          ...
          if (cleanNumber) {
            ...
            if (phoneInputRef.current) {
              ...
              setFormattedPhone(
                `+${callingCode}${cleanNumber.replace(/^0/, "")}`,
              );
              (phoneInputRef.current as any).setState({ number: cleanNumber });
            }
          }
        } else {
          Alert.alert(
            "Notice",
            "This contact doesn't have an associated phone number.",
          );
        }
      }
    } catch (error) {
  ...
  const handleGenerateCode = async () => {
    ...
    if (!formattedPhone.trim())
      return Alert.alert("Error", "Enter a phone number");
    ...
    if (!checkValid) {
      Alert.alert(
        ...
        "The phone number provided is incorrect for the selected country structure.",
      );
      return;
    }
    ...
    try {
      ...
      if (guestImage) {
        const url = await getS3UploadedUrl(guestImage, "Invitation-Images");
      ...
      const payload = {
        ...
        guest_phone: formattedPhone ? formattedPhone.trim() : null,
        ...
        permitted_days: guestType === "staff_entry" ? permittedDays : [],
      };
