...
export default function Community() {
  ...
  const [localLastPostRead, setLocalLastPostRead] = useState<
    Record<string, string>
  >({});
  const [likes, setLikes] = useState<Like[]>([]);
  const STORAGE_KEY = "GM_READ_POST_IDS";
  const [sessionReadPostIds, setSessionReadPostIds] = useState<string[]>([]);
    ...
    if (selectedEstateId) {
      const syncFeedData = async () => {
        try {
          ...
          if (savedIds !== null) {
            setSessionReadPostIds(JSON.parse(savedIds));
          } else {
            // Initialize it if it doesn't exist yet
            await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify([]));
            setSessionReadPostIds([]);
          }
          const readData = await fetchLastPostRead();
          if (readData.success) {
            setLocalLastPostRead(readData.last_post_read);
          }

          await loadPosts();
          await updateLastPostRead(selectedEstateId);
        } catch (err) {
          console.error("Error syncing feed data:", err);
        }
      };
  ...
  const handleRefresh = useCallback(
    async (shouldScrollToTop = false) => {
      ...
      try {
        // 1. Concurrently fetch both your read markers and your posts layout in ONE shot
        const [readData, freshPosts] = await Promise.all([
          fetchLastPostRead(),
          communityApi.getPosts(selectedEstateId),
        ]);

        // 2. Hydrate the unread timestamp baseline context
        if (readData.success) {
          setLocalLastPostRead(readData.last_post_read);
        }

        // 3. Set the posts immediately from the array unpack
        setPosts(freshPosts);

        // 4. Update the backend marker silently
        await updateLastPostRead(selectedEstateId);
      } catch (err) {
        console.error(
          "Failed to sync fresh context data during pull-refresh:",
          err,
        );
      } finally {
  ...
  const handleOpenPost = async (post: Post) => {
    if (!sessionReadPostIds.includes(post.id)) {
      const updatedIds = [...sessionReadPostIds, post.id];
      setSessionReadPostIds(updatedIds);
      try {
        await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(updatedIds));
      } catch (err) {
        console.error(
          "Failed processing device background write metric sync:",
          err,
        );
      }
    }
