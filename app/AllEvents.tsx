...
export default function AllEventsScreen() {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        setLoading(true);
        const data = await getOrganizerEvents(idToFetch.toString());
        setEvents(data);
    ...
    <View className={`flex-1 p-6 ${isDarkMode ? "bg-slate-950" : "bg-white"}`}>
      ...
      {selectedEstateId && (
        ...
        >
          ...
          <Text
            className={`text-[11px] font-oswald-semibold ml-1 mr-2 uppercase tracking-wide ${isDarkMode ? "text-gm-gold" : "text-indigo-950"}`}
          >
            {activeEstateFilterName}
          </Text>
          {/* <TouchableOpacity onPress={() => setSelectedEstateId(null)}>
            <Text
              className={`font-bold text-xs px-1 ${isDarkMode ? "text-red-400" : "text-indigo-400"}`}
            >
              ✕
            </Text>
          </TouchableOpacity> */}
      ...
      <Modal visible={showEstateFilterModal} animationType="slide" transparent>
        <View className="flex-1 bg-black/50 justify-end">
          <View
            className={`h-[50%] rounded-t-[3rem] p-6 border-t ${isDarkMode ? "bg-slate-900 border-gm-gold" : "bg-white"}`}
          >
          ...
          <View className="flex-1 bg-black/95">
            ...
            <ScrollView className="px-6" showsVerticalScrollIndicator={false}>
              ...
              >
                ...
                <View className="p-8">
                  <View className="flex-row justify-between items-start mb-4">
                    <View className="flex-1">
                      <Text
                        className={`text-3xl font-black leading-tight ${isDarkMode ? "text-white" : "text-slate-900"}`}
                      >
                        {selectedEvent.title}
                      </Text>
                      <View
                        className={`self-start px-4 py-1 rounded-full mt-2 border ${
                          isDarkMode
                      ...
                      >
                        <Text
                          className={`font-black text-[10px] uppercase tracking-widest ${isDarkMode ? "text-gm-gold" : "text-white"}`}
                        >
                  ...
                  <Text
                    className={`font-medium leading-relaxed mb-6 ${isDarkMode ? "text-slate-300" : "text-slate-600"}`}
                  >
                ...
                <View
                  className={`p-6 border-t items-center ${isDarkMode ? "border-slate-800" : "border-slate-100"}`}
                >
                  <Text className="text-slate-400 font-black text-[9px] uppercase tracking-widest mb-1">
                    Register at
                  </Text>
                  <Text
                    className={`font-black text-sm lowercase ${isDarkMode ? "text-gm-gold" : "text-indigo-600"}`}
                  >
              ...
              {!isSharing && (
                <>
                  <View
                    className={`mt-8 p-3 border-t border-b ${isDarkMode ? "border-slate-800" : "border-slate-100"}`}
                  >
                    <Text className="text-slate-400 font-black text-[10px] uppercase">
                      STATUS
                    </Text>
                    <Text
                      className={`font-black text-lg ${isDarkMode ? "text-gm-gold" : "text-indigo-600"}`}
                    >
                  ...
                  <View
                    className={`mt-1 p-3 border-b ${isDarkMode ? "border-slate-800" : "border-slate-100"}`}
                  >
                    <Text
                      className={`font-bold ${isDarkMode ? "text-slate-200" : "text-indigo-900"}`}
                    >
...
function InfoBox({ icon, label, value, isDarkMode }: InfoBoxProps) {
  return (
    <View className="w-1/2 flex-row items-start mb-4">
      <View
        className={`p-3 rounded-2xl ${isDarkMode ? "bg-slate-900" : "bg-indigo-50"}`}
      >
        {icon}
      </View>
      <View className="ml-3 flex-1">
        <Text
          className={`font-black text-[9px] uppercase tracking-tighter ${isDarkMode ? "text-gm-gold" : "text-slate-400"}`}
        >
          {label}
        </Text>
        <Text
          className={`font-bold text-xs leading-4 ${isDarkMode ? "text-white" : "text-slate-800"}`}
        >
          {value || "N/A"}
        </Text>
      </View>
