...
export default function JoinRequestForm() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        Alert.alert(
          "System Error",
          "Could not synchronize baseline properties.",
        );
      }
    })();
  }, []);
  ...
  const handleCapture = async (
    ...
    if (!permission.granted)
      return Alert.alert("Required", "Camera permissions are required.");
  ...
  const handleCaptureContractImage = async (
    flatTargetIndex: number,
    bName: string,
    uName: string,
  ) => {
    try {
      ...
      if (!result.canceled) {
        ...
        nextContracts[flatTargetIndex] = {
          uri: result.assets[0].uri,
          name: `contract_${bName}_${uName}.jpg`,
          mimeType: "image/jpeg",
          blockTarget: bName,
          unitTarget: uName,
        };
      ...
      Alert.alert("Camera Error", "Could not capture image document.");
    }
  };

  const stepChain = [
    "estate_details",
    activeConfig.selfie && "selfie",
    activeConfig.ids && "identification",
    activeConfig.utility_bill && "address_bill",
    activeConfig.rent_contract && "rent_contract",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (activeConfig.utility_bill && utilityBill)
      ...
      if (response.success) {
        Alert.alert(
          "Success",
          "Request transmitted into verification sequence.",
          [
            {
              text: "OK",
              onPress: () => {
      ...
      Alert.alert(
        "Submission Error",
        err.message || "Failed validating registration files.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <ScrollView className="flex-1 bg-[#121826] px-3">
      ...
      {currentViewKey === "estate_details" && (
        <View className="bg-[#1e293b] p-5 rounded-2xl border border-[#334155] mb-8 pb-10">
          ...
          <View className="mb-6">
            <Text className="text-slate-400 mb-2 text-xs font-semibold uppercase tracking-wider">
              Target Estate Hub
            </Text>
            <TouchableOpacity
            ...
            >
              <Text
                className={
                  estateLabel ? "text-slate-200 font-medium" : "text-slate-500"
                }
              >
          ...
          <Text className="text-slate-300 font-bold text-sm mb-3">
            Unit Location Allocations
          </Text>

          {locations.map((blockNode, bIdx) => (
            <View
              key={bIdx}
              className="p-4 bg-[#0f172a] border border-[#334155] rounded-2xl mb-4"
            >
              <View className="flex-row justify-between items-center mb-3">
                <Text className="text-[11px] font-bold text-indigo-400 tracking-wider uppercase">
                  Block Domain Cluster #{bIdx + 1}
                </Text>
                {locations.length > 1 && (
                  <TouchableOpacity
                    onPress={() => removeBlockNode(bIdx)}
                    className="p-1"
                  >
              ...
              <Text className="text-[10px] font-bold text-slate-500 uppercase mb-2 tracking-wide">
                Assigned Units in Block
              </Text>
              ...
              {blockNode.units.map((unitNode, uIdx) => (
                <View key={uIdx} className="flex-row items-center gap-x-2 mb-2">
                  ...
                  <TouchableOpacity
                    onPress={() => removeUnitFromBlock(bIdx, uIdx)}
                    className="p-2"
                  >
              ...
              >
                ...
                <Text className="text-indigo-400 Regal text-[11px] font-bold">
                  Add Unit to this Block
                </Text>
              </TouchableOpacity>
            </View>
          ))}
          ...
          <TouchableOpacity
            onPress={addBlockNode}
            className="flex-row items-center justify-center p-4 border border-dashed border-[#4f46e5] rounded-xl mb-6 bg-[#4f46e5]/10"
          >
            <Plus size={16} color="#818cf8" className="mr-1.5" />
            <Text className="text-indigo-400 font-bold text-xs">
              Add Another Separate Block
            </Text>
          </TouchableOpacity>
          ...
          >
            <Text
              className={`font-bold ${isStep1Valid ? "text-white" : "text-slate-500"}`}
            >
      ...
      {currentViewKey === "selfie" && (
        <View className="bg-[#1e293b] p-6 rounded-2xl border border-[#334155]">
          ...
          <Text className="text-slate-400 text-xs mb-4">
            Please take a clear front face capture under bright lighting.
          </Text>

          <TouchableOpacity
            onPress={() => handleCapture("selfie")}
            className="border-dashed border-2 border-[#475569] h-56 rounded-2xl items-center justify-center bg-[#0f172a] overflow-hidden"
          >
            {selfie ? (
              <Image source={{ uri: selfie }} className="w-full h-full" />
            ) : (
              <Text className="text-indigo-400 font-semibold">
                📸 Engage System Camera
              </Text>
            )}
          </TouchableOpacity>
          ...
          <View className="flex-row justify-between mt-8">
            ...
            <TouchableOpacity
              disabled={!selfie}
              onPress={() =>
                step === stepChain.length ? handleSubmit() : setStep(step + 1)
              }
              className={`py-4 px-10 rounded-xl ${selfie ? "bg-indigo-600" : "bg-[#334155]"}`}
            >
              <Text
                className={`font-bold ${selfie ? "text-white" : "text-slate-500"}`}
              >
                Continue
              </Text>
            </TouchableOpacity>
      ...
      {currentViewKey === "identification" && (
        <View className="bg-[#1e293b] p-6 rounded-2xl border border-[#334155]">
          ...
          <TouchableOpacity
            ...
            className="flex-row items-center mb-4 p-3 bg-[#0f172a] rounded-xl border border-[#334155]"
          >
            ...
            <Text className="text-slate-200 font-medium text-sm">
              National ID Slate Slip (NIN)
            </Text>
          </TouchableOpacity>

          <TouchableOpacity
            onPress={() => setIdType("voters")}
            className="flex-row items-center mb-6 p-3 bg-[#0f172a] rounded-xl border border-[#334155]"
          >
            <View className="w-5 h-5 rounded-full border-2 border-indigo-500 mr-3 items-center justify-center">
              {idType === "voters" && (
                <View className="w-2.5 h-2.5 rounded-full bg-indigo-500" />
          ...
          <View className="space-y-3">
            <TouchableOpacity
              ...
              className="border border-dashed border-[#475569] rounded-xl p-4 bg-[#0f172a] items-center"
            >
              <Text className="text-slate-300 text-xs font-semibold">
                {idFront ? "✅ Front Frame Logged" : "📸 Capture ID Front View"}
              </Text>
            </TouchableOpacity>
            {idType !== "nin" && (
              <TouchableOpacity
                onPress={() => handleCapture("idBack")}
                className="border border-dashed border-[#475569] rounded-xl p-4 mt-3 bg-[#0f172a] items-center"
              >
                <Text className="text-slate-300 text-xs font-semibold">
                  {idBack ? "✅ Back Frame Logged" : "📸 Capture ID Rear View"}
                </Text>
              </TouchableOpacity>
            )}
          </View>
          ...
          <View className="flex-row justify-between mt-8">
            ...
            <TouchableOpacity
              ...
              className={`py-4 px-10 rounded-xl ${isIdValid ? "bg-indigo-600" : "bg-[#334155]"}`}
            >
              <Text
                className={`font-bold ${isIdValid ? "text-white" : "text-slate-500"}`}
              >
                Continue
              </Text>
            </TouchableOpacity>
      ...
      {currentViewKey === "address_bill" && (
        <View className="bg-[#1e293b] p-6 rounded-2xl border border-[#334155]">
          <Text className="text-lg font-bold mb-2 text-white">
            Address Verification Invoice
          </Text>
          <Text className="text-slate-400 text-xs mb-4">
            Upload a recent utility receipt verifying resident location matching
            details.
          </Text>

          <TouchableOpacity
            onPress={() => handleCapture("utility")}
            className="border-dashed border-2 border-[#475569] h-48 rounded-xl items-center justify-center bg-[#0f172a] overflow-hidden"
          >
            {utilityBill ? (
              <Image source={{ uri: utilityBill }} className="w-full h-full" />
            ) : (
              <Text className="text-slate-400">
                Scan Utility Receipt Invoice
              </Text>
            )}
          </TouchableOpacity>
          ...
          <View className="flex-row justify-between mt-8">
            ...
            <TouchableOpacity
              ...
              className={`py-4 px-10 rounded-xl ${utilityBill ? "bg-indigo-600" : "bg-[#334155]"}`}
            >
              <Text
                className={`font-bold ${utilityBill ? "text-white" : "text-slate-500"}`}
              >
                Continue
              </Text>
            </TouchableOpacity>
      ...
      {currentViewKey === "rent_contract" && (
        <View className="bg-[#1e293b] p-5 rounded-2xl border border-[#334155] mb-8">
          <Text className="text-lg font-bold mb-1 text-white">
            Tenancy Agreements Indexing
          </Text>
          <Text className="text-slate-400 mb-5 text-xs">
            Attach a verification file (PDF or Image scan) matching each unit
            entry.
          </Text>
          ...
          {(() => {
            ...
            return locations.map((bObj) =>
              bObj.units.map((uName) => {
                ...
                const selectedFile = contracts[currentFlatIndex];

                return (
                  <View
                    key={`${currentBlockLabel}-${currentUnitLabel}-${currentFlatIndex}`}
                    className="mb-5 p-4 bg-[#0f172a] border border-[#334155] rounded-xl"
                  >
                    <View className="flex-row items-center justify-between mb-3">
                      ...
                      {selectedFile && (
                        <Text className="text-[10px] font-bold bg-[#334155] text-slate-300 px-2 py-0.5 rounded uppercase">
                    ...
                    <View className="flex-row gap-x-2 mb-3">
                      <TouchableOpacity
                        onPress={() =>
                          handlePickContractPDF(
                            currentFlatIndex,
                            currentBlockLabel,
                            currentUnitLabel,
                          )
                        }
                        className={`flex-1 flex-row items-center justify-center p-3 rounded-xl border border-[#334155] gap-x-2 bg-[#1e293b] active:bg-[#334155]`}
                      >
                        <FileUp size={14} color="#a5b4fc" />
                    ...
                    {/* Upload Status Label / Mini Preview */}
                    {selectedFile && (
                      <View className="flex-row items-center justify-between bg-[#064e3b]/20 border border-emerald-500/30 rounded-xl p-3 mt-1">
                        <Text
                          className="text-xs text-emerald-400 font-medium flex-1 mr-2"
                          numberOfLines={1}
                        >
                          {selectedFile.name}
                        </Text>
          ...
          <View className="flex-row justify-between mt-6">
            ...
            >
              {loading ? (
                ...
                <Text
                  className={`font-bold ${areAllContractsUploaded ? "text-white" : "text-slate-500"}`}
                >
                  Submit Application
                </Text>
              )}
