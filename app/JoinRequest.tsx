...
export default function JoinRequestForm() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      // We use FormData because we are sending binary files (images)
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50 p-4">
        ...
        <View className="bg-white p-6 rounded-2xl shadow-sm">
          ...
          >
            {utilityBill ? (
              ...
              <Text className="text-gray-400">Capture Bill Photo</Text>
            )}
          </TouchableOpacity>
          <View className="flex-row justify-between mt-8">
            <TouchableOpacity onPress={prevStep}>
      ...
      {/* STEP 4: ESTATE SELECTION (Existing Logic) */}
      {step === 4 && (
        <View>
          ...
          <View style={{ marginBottom: 16 }}>
            <Text style={{ marginBottom: 4 }}>Block</Text>
            <TextInput
              value={block}
          ...
          <View style={{ marginBottom: 16 }}>
            <Text style={{ marginBottom: 4 }}>Unit</Text>
            <TextInput
              value={unit}
