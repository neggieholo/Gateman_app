...
export default function LoginScreen() {
  ...
  const [formattedPhone, setFormattedPhone] = useState<string>("");
  const [isFirstSubUserSetup, setIsFirstSubUserSetup] =
    useState<boolean>(false);
  const [subUserSetupStep, setSubUserSetupStep] = useState<
    "EMAIL_OTP" | "PHONE_COLLECT"
  >("EMAIL_OTP");
  ...
  const deviceMeta = {
    phone: [Device.osName?.split("/")[0]],
    platform: Platform.OS,
    model: Device.modelName,
    version: Device.osVersion,
    isTablet: Device.deviceType === 2,
  };
  ...
  const handleLogin = async (
    ...
    try {
      ...
      if (response.success) {
        ...
        await clearStaleCacheOnStartup();
  ...
  const handleOtpVerify = async (finalOtp: string) => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/admin/verify-otp-only`, {
        method: "POST",
    ...
    >
      <SafeAreaView className="flex-1 bg-gm-navy">
        ...
        >
          ...
          <Modal
            visible={
              isFirstSubUserSetup && subUserSetupStep === "PHONE_COLLECT"
            }
            animationType="fade"
            transparent={true}
          >
            <View className="flex-1 justify-center items-center bg-black/70 px-6">
              <View className="bg-white w-full rounded-3xl p-8 items-center shadow-lg">
                <Text className="text-2xl font-black text-gm-navy mb-2 text-center">
                  Complete Profile
                </Text>
                <Text className="text-gray-500 text-center mb-6 text-xs font-medium leading-relaxed">
                  Your sub-account requires a verified phone number layout
                  configured to monitor security checkpoints.
                </Text>
                <View className="mb-4">
                  <PhoneInput
                    ref={phoneInputRef}
                ...
                {otpLoading && verifyingField === "phone" ? (
                  ...
                  <TouchableOpacity
                    onPress={() => handleRequestOtp(phone, "phone")}
                    className="w-full bg-gm-navy py-4 rounded-2xl items-center shadow-sm mb-4"
                  >
                ...
                >
                  <Text className="text-sm font-bold text-red-500 mt-2">
                    Log Out / Cancel
                  </Text>
                </TouchableOpacity>
              </View>
