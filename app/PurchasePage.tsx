...
import { ChevronLeft, CheckCircle2, Zap, Smartphone, Wifi, Search } from "lucide-react-native";
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <View className="px-6 py-4 flex-row items-center border-b border-gray-50">
        ...
        <Text className="text-xl font-black text-gray-900 capitalize">{type} Purchase</Text>
      </View>

      <ScrollView className="flex-1 p-6" showsVerticalScrollIndicator={false}>
        
        {/* 1. Provider Selection */}
        <Text className="text-xs font-bold text-gray-400 uppercase mb-3">Select Provider</Text>
          ...
          {(type === 'electricity' ? electricityProviders : telcos).map((p) => (
            <TouchableOpacity
              ...
              className={`p-3 rounded-2xl border-2 items-center justify-center w-[22%] ${
                form.provider === p.id ? 'border-indigo-600 bg-indigo-50' : 'border-gray-100 bg-gray-50'
              }`}
            >
              <View className={`w-10 h-10 ${p.color} rounded-full items-center justify-center mb-1`}>
                <Text className="text-white font-bold text-[8px]">{p.name.substring(0,3)}</Text>
              </View>
              <Text className="text-[10px] font-bold text-gray-600" numberOfLines={1}>{p.name}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View className="flex-row items-center bg-gray-50 rounded-2xl border border-gray-100 px-4 py-1 mb-4">
          <TextInput
            className="flex-1 py-3 text-gray-900 font-bold"
            placeholder={type === 'electricity' ? "Enter 11-digit Meter No" : "08012345678"}
        ...
        {type === 'electricity' && verifiedName && (
          <View className="bg-emerald-50 p-4 rounded-xl mb-4 border border-emerald-100 flex-row items-center">
            ...
            <Text className="ml-2 text-emerald-800 font-bold text-xs">{verifiedName}</Text>
          </View>
        )}
        ...
        {type !== 'data' && (
          <View>
            <Text className="text-xs font-bold text-gray-400 uppercase mb-2">Amount (₦)</Text>
            <TextInput
              className="bg-gray-50 rounded-2xl border border-gray-100 px-4 py-4 text-gray-900 font-black text-xl mb-6"
              placeholder="0.00"
              keyboardType="number-pad"
              value={form.amount}
        ...
        {type === 'data' && (
          <View className="mb-6">
            <Text className="text-xs font-bold text-gray-400 uppercase mb-3">Select Plan</Text>
            {/* Example Plans */}
            {[
              ...
              <TouchableOpacity
                key={v.code}
                onPress={() => setForm({ ...form, variation: v, amount: v.price })}
                className={`p-4 mb-2 rounded-2xl border-2 flex-row justify-between items-center ${
                  form.variation?.code === v.code ? 'border-indigo-600 bg-indigo-50' : 'border-gray-100 bg-gray-50'
                }`}
              >
        ...
        <TouchableOpacity
          ...
          className={`py-4 rounded-2xl shadow-lg mb-10 ${loading ? 'bg-gray-400' : 'bg-indigo-600'}`}
        >
          {loading ? (
            <ActivityIndicator color="white" />
          ) : (
            <Text className="text-white text-center font-black text-lg">
              Confirm {type === 'electricity' ? 'Token' : 'Purchase'}
            </Text>
          )}
        </TouchableOpacity>
